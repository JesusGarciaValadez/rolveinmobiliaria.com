export default {
  ...
  methods: {
    ...
    getExpedientInfo: function () {
      ...
      if (expedientId !== '') {
        ...
        axios.get(uri, inicialization)
          ...
          .then(response => {
            this.client.id = response.id || ''
            this.client.expedient = response.expedient || ''
    ...
    setClosingContract: function () {
      ...
      this.sale.closingContract.exclusivity_contract = this.getID('SCC_exclusivity_contract').checked
      this.sale.closingContract.publication = this.getID('SCC_publication').checked
